...
def h_fn(node, goal, h_fn_val):
        if h_fn_val=='octile':
            ...
            return dist
        elif h_fn_val == 'manhattan':
            h_trans = np.abs(node.position[0] - goal.position[0]) + np.abs(node.position[1] - goal.position[1])
            h_rot = (np.min(np.array(np.abs(node.position[2]-goal.position[2]), 
                                    2 * np.pi - np.abs(node.position[2] - goal.position[2]))))**2
            dist = np.sqrt(h_rot + h_trans)
            return dist

        elif h_fn_val == 'chebyshev':
            xdiff = np.abs(node.position[0] - goal.position[0])
            ydiff = np.abs(node.position[1] - goal.position[1])
            h_rot = (np.min(np.array(np.abs(node.position[2]-goal.position[2]), 
                                    2 * np.pi - np.abs(node.position[2] - goal.position[2]))))**2
            h_tran = np.max([xdiff, ydiff])
            dist = np.sqrt(h_rot + h_tran)
            return dist
...
def report_e(term, atleast_one_path, start_time):
    if term and not atleast_one_path:
        print("Time for initial slution: ", time.time()-start_time)
        print("path_found!!!")
...
def anastar(env, goal_config, incrmts, h_fn_val='euclidean'):
    ...
    while(len(open_list)!=0):
        ...
        report_e(term, atleast_one_path, start_time)
        if term:
